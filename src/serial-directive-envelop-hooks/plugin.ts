...
export const useSerialDirective = (): Plugin => {
    return {
        onExecute({ args }) {
            ...
            const createWrappedResolver = (originalResolver: GraphQLFieldResolver<any, any> | undefined): GraphQLFieldResolver<any, any> => {
                return (source, fieldArgs, fieldContext, info) => {
                    ...
                    const isRootField = !info.path.prev || info.path.prev.key === undefined;
