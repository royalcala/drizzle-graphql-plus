...
async function seed() {
  console.log("🌱 Seeding DataLoader test database...");
  console.log(
  ...
  console.log("✨ DataLoader test database seeded successfully!");

  client.close();
}

seed().catch((error) => {
  console.error("❌ Error seeding DataLoader test database:", error);
  process.exit(1);
});