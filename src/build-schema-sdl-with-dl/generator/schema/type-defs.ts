import { Column, is, One } from "drizzle-orm";
...
const columnToSDL = (
  ...
  if ((column as any).customGraphqlType) {
    ...
    // If this is a primary key and no custom type is set, use ID
    baseType = "ID";
  } else {
...
export const generateTypeDefs = (
  ...
  for (const [tableName, tableRelations] of Object.entries(relations)) {
    ...
    for (const [relationName, relationInfo] of Object.entries(tableRelations)) {
      ...
      for (let i = 0; i < config.fields.length; i++) {
        ...
        // Check if the referenced column is a primary key (which would map to ID)
        const referencedIsPrimaryKey =
          (referencedColumn as any).primary || false;

        if (!foreignKeyHasCustomType) {
          if (referencedCustomType) {
            // If referenced column has a custom GraphQL type, use it
            foreignKeyTypes.set(
              `${tableName}.${foreignKeyPropertyName}`,
              referencedCustomType
            );
          } else if (referencedIsPrimaryKey) {
            // If referenced column is a primary key (maps to ID), foreign key should also be ID
  ...
  for (const [tableName, tableInfo] of Object.entries(tables)) {
    ...
    for (const [columnName, column] of Object.entries(tableInfo.columns)) {
      const isPrimaryKey = (column as any).primary || false;
